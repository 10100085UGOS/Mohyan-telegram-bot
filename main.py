...
# =============================================================================
# ADVANCED WHITELIST SYSTEM (USER-FRIENDLY + ANIMATED)
# =============================================================================

# ----- Helper to track user chats (must be placed before any whitelist commands) -----
@bot.message_handler(func=lambda m: True)
def track_user_chats(message):
...
# ----- /add_whitelist – show list and add -----
...
# ----- /remove_whitelist – owner only, interactive list -----
...
def whitelist_remove_callback(call):
    if call.from_user.id != OWNER_ID:
        bot.answer_callback_query(call.id, "❌ Not owner")
        return
    chat_id = int(call.data.split('_')[2])
    conn = get_db()
...
# ----- /remove_unknown_channels – leave all non-whitelisted with progress bar -----
